      ...
      <div style={{ background:"#fff",borderRadius:12,border:`1px solid ${C.border}`,overflow:"hidden" }}>
        ...
        <div style={{ padding:"20px 22px" }}>
          ...
          {activeTab==="Import History" && (()=>{
            ...
            const inputStyle = { width:"100%", border:`1px solid ${C.border}`, borderRadius:6, padding:"5px 8px", fontSize:12, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const, color:C.text, background:"#fff" };
            const selStyle   = { ...inputStyle, color:C.slate, cursor:"pointer" };
            return (
            <div>
              {/* Title + Import button */}
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
                <div style={{ fontSize:22,fontWeight:800,color:C.navy,letterSpacing:"-0.02em" }}>Imports History</div>
                <label style={{ padding:"10px 32px",borderRadius:9,border:"none",background:"#93C5FD",color:"#1E3A5F",fontSize:14,fontWeight:700,cursor:"pointer",display:"inline-flex",gap:6,alignItems:"center",letterSpacing:"0.01em" }}>
                  <input type="file" accept=".csv,.xlsx" style={{ display:"none" }} onChange={e=>{if(e.target.files?.[0]) alert("Import started: "+e.target.files[0].name); (e.target as HTMLInputElement).value="";}}/>
                  Import
                </label>
              </div>
                  ...
                  <thead>
                    ...
                    <tr style={{ background:"#F8FAFC",borderBottom:`1px solid ${C.border}` }}>
                      {["Name","Type","Imported / Total","Date & Time","Responsible","Status",""].map(h=>(
                        <th key={h} style={{ padding:"11px 14px",textAlign:"left",fontSize:12,fontWeight:600,color:C.navy,whiteSpace:"nowrap",userSelect:"none" as const,cursor:h?"pointer":"default" }}>
              ...
              <div style={{ marginTop:14,paddingTop:12,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                  <span style={{ fontSize:13,color:C.text }}>Page <span style={{ color:C.primary,fontWeight:700,cursor:"pointer" }}>1</span> of 1</span>
