...
The **Processing & Feedback** stepper (board `rB069`) is the main producer of
entries in this feed. The matrix below maps each P&F action to the log entry it
writes into the Activities tab and to the resulting lead status (from the
status table embedded on `rB069`).

| # | P&F Step | Action / control | → Activities-tab log entry | Type | Badge | Key fields | Status result (Lifecycle · Status · Processing) |
|---|----------|------------------|---------------------------|------|-------|-----------|--------------------------------------------------|
| 1 | 1 · Send Initial Message | Email template → **Mark as Sent & Continue** | Email (Sent) | ✉️ | Logged | From · To · Subject · Body · Attachments | Lead · In Contact · *Initial message sent* |
| 2 | 1 · Send Initial Message | SMS/WhatsApp template → **Mark as Sent & Continue** | Offline | 📝 | Logged | Logged By · Type=WhatsApp/SMS · Description | Lead · In Contact · *Initial message sent* |
| 3 | 1 · Send Initial Message | **Skip — call directly** | Update (status only) | ✎ | — | "Skipped — calling directly" | Lead · In Contact |
| 4 | 2 · Call Attempts | Attempt *n* = **Not Reached** → Save & Continue | Outbound Call | 📤 | Logged | Call By · Duration · Call report | Lead · In Contact · *Not reached after n attempts* (n = 1–4) |
| 5 | 2 · Call Attempts | 5th attempt **Not Reached** (auto-finalize) | Outbound Call | 📤 | Logged | " | Lead · **Not Reached** · *Not reached after 5 attempts* |
| 6 | 2 · Call Attempts | Attempt *n* = **Reached** | Outbound Call | 📤 | Logged | " | Lead · In Contact · *Reached after n attempts* (1–5) |
| 7 | 2 · Call Attempts | **Create a Task** (callback reminder) | Task | ☑️ | — → Overdue | Created By · Created At · Priority · Description | *(status unchanged)* |
| 8 | 2 · Call Attempts | Missed-call message copied & sent (SMS/WA) | Offline *(optional)* | 📝 | Logged | Logged By · Type · Description | *(status unchanged)* |
| 9 | 3 · Call Outcome | **Appointment Scheduled** → Schedule & Continue | Appointment (scheduled) | 📅 | — | Hosted By · Type · Attendees · Location/Link · Description · Attachments | Lead · **Appointment** · *Appointment Scheduled* |
| 10 | 3 · Call Outcome | **Not Interested** (+ Note) | Update (+ Note on call) | ✎ | — | Note → call report | Lead · Not Interested · *Not interested during the call* |
| 11 | 3 · Call Outcome | **Currently Not Interested** | Update | ✎ | — | Note | Lead · Currently Not Interested · *… during the call* |
| 12 | 3 · Call Outcome | **Difficult Case** | Update | ✎ | — | Note | Lead · Difficult Case · *… during the call* |
| 13 | 3 · Call Outcome | **Other** (neutral) | Update | ✎ | — | Note | Lead · In Contact *(no DNC)* |
| 14 | 4 · Appointment Outcome | **Qualified / Won** (+ Note) | Appointment updated → **Outcome + Note** | 📅 | Logged | Outcome=Qualified · Note | Lead · **Qualified** · *Appointment completed* |
| 15 | 4 · Appointment Outcome | **Reschedule** (stays on step) | Appointment re-booked | 📅 | — | new date/time (reschedule count) | Lead · Appointment |
| 16 | 4 · Appointment Outcome | **Attending Event** | Appointment → Outcome | 📅 | Logged | Outcome=Attending Event | Lead · **Follow Up** · *Attending event* |
| 17 | 4 · Appointment Outcome | **Not Interested** | Appointment → Outcome + Update | 📅 ✎ | Logged | Outcome · Note | Lead · Not Interested · *… during appointment* |
| 18 | 4 · Appointment Outcome | **Currently Not Interested** | Appointment → Outcome + Update | 📅 ✎ | Logged | Outcome · Note | Lead · Currently Not Interested · *… during appointment* |
| 19 | 4 · Appointment Outcome | **Difficult Case** | Appointment → Outcome + Update | 📅 ✎ | Logged | Outcome · Note | Lead · Difficult Case · *… during appointment* |
| 20 | 5 · Finalize Process | **Mark lead as processed** | Update (status) | ✎ | — | — | Lead · *(keeps outcome status)* · **Process completed** |
| 21 | 5 · Finalize Process | **Do Not Contact** toggle on/off | Update (DNC flag) | ✎ | — | DNC on/off logged | *(status unchanged)* |
| 22 | 5 · Finalize Process | **Add to My Network** (Convert) | Update (lifecycle) | ✎ | — | Network Status | **Network** · Customer / Prospect / Partner |

### Coverage vs. this feed's log types

| Activities-tab log type | Produced by P&F? | Where |
|---|---|---|
| ✉️ Email — **Sent** | ✅ | Step 1 (email channel) |
| ✉️ Email — Received | ❌ | inbound only, outside P&F |
| 📤 Call — **Outbound** | ✅ | Step 2 attempts |
| 📥 Call — Inbound | ❌ | inbound only, outside P&F |
| 📅 Appointment (scheduled + **logged w/ outcome**) | ✅ | Steps 3 & 4 |
| ☑️ Task | ✅ | Step 2 (callback) |
| 📝 Offline | ✅ | Step 1 (SMS/WhatsApp), Step 2 (missed-call msg) |
| ✎ Update (status / DNC / conversion) | ✅ | every step (status table rows 2–30) |

P&F generates **six of the eight** log variants; the two it never creates are
*Inbound Call* and *Received Email* (both inbound/system events). Which entries
are auto-written (rows 3, 8, 10–13, 20–22) is an inference where `rB069` does
not state it outright — confirm with the owner of the log-write rules.
