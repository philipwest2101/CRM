...
const TYPE_SCHEMA: Record<string, {
  ...
  Financing: {
    fields: [
      ...
      { key: "financingTypeOther", label: "Type of Financing", control: "text", placeholder: "Enter type of financing", showIf: d => d.financingType === "Other" },
      { key: "partner", label: "Product Partner", control: "text", placeholder: "Enter product partner" },
      { key: "amount", label: "Financing Amount", control: "text", placeholder: "Enter financing amount" },
      { key: "term", label: "Term", control: "text", placeholder: "Enter term" },
      { key: "closingDate", label: "Closing Date", control: "date" },
    ],
    card: [
      { label: "Financing Amount", key: "amount" },
      { label: "Term", key: "term" },
  ...
  Insurance: {
    fields: [
      ...
      { key: "partner", label: "Product Partner", control: "text", placeholder: "Enter product partner" },
      { key: "amount", label: "Contract Amount", control: "text", placeholder: "Enter contract amount" },
  ...
  Investment: {
    fields: [
      ...
      { key: "investmentTypeOther", label: "Type of Investment", control: "text", placeholder: "Enter type of investment", showIf: d => d.investmentType === "Other" },
      { key: "title", label: "Product Title", control: "text", placeholder: "Enter product title" },
      { key: "partner", label: "Product Partner", control: "text", placeholder: "Enter product partner" },
      { key: "oneTime", label: "One-time Investment Amount", control: "text", placeholder: "Enter one-time investment amount" },
      { key: "savings", label: "Savings Plan Amount", control: "text", placeholder: "Enter saving plan amount" },
      { key: "closingDate", label: "Closing Date", control: "date" },
    ],
    card: [
  ...
  Workshop: {
    fields: [
      ...
      { key: "attended", label: "Attended", control: "segmented", options: ["Yes", "No"] },
      { key: "paid", label: "Paid", control: "segmented", options: ["Yes", "No"] },
    ],
    card: [
    ...
    defaults: { attended: "Yes", paid: "Yes" },
  },
  Advisory: {
    fields: [
...
// ── segmented control (Type of Financing, Status, Yes/No …) ───────────────────
...
const ProductModal = ({ mode, type, product, onClose, onSave }: any) => {
  ...
  const [draft, setDraft] = useState<Product>(() =>
    product
      ? { ...product }
      : { id: "", type, status: "Active", note: "", ...(schema.defaults || {}) }
  );
  ...
  const renderField = (f: Field) => {
    ...
    if (f.control === "segmented") {
      ...
      control = <textarea style={{ ...fieldStyle, minHeight: 80, resize: "vertical", lineHeight: 1.5 }} placeholder={f.placeholder} value={draft[f.key] || ""} onChange={e => set(f.key, e.target.value)} />;
    } else {
      control = <input style={fieldStyle} placeholder={f.placeholder} value={draft[f.key] || ""} onChange={e => set(f.key, e.target.value)} />;
    }
    return (
  ...
  return (
    <ModalShell icon="📦" title={mode === "edit" ? "Edit Product" : "Add Product"} subtitle={type} accent={C.primary} width={480} onClose={onClose}>
      ...
      <div>
        <Label>Note</Label>
        <textarea style={{ ...fieldStyle, minHeight: 80, resize: "vertical", lineHeight: 1.5 }} placeholder="Enter a note"
          value={draft.note || ""} onChange={e => set("note", e.target.value)} />
      </div>
      <FooterBtns onClose={onClose} label={mode === "edit" ? "Update" : "Save"} onAction={() => { onSave(draft); onClose(); }} />
    </ModalShell>
  );
};

// ── summary card ──────────────────────────────────────────────────────────────
const ProductCard = ({ product, onEdit }: any) => {
  ...
  return (
    <div style={{ border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 16px", background: "#fff" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <IconBtn title={open ? "Collapse" : "Expand"} onClick={() => setOpen(o => !o)}>{open ? "▲" : "▼"}</IconBtn>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 14 }}>
        {schema.card.map(cell => (
          <div key={cell.key} style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{product[cell.key] || "—"}</div>
          </div>
        ))}
      </div>
...
export const ProductsTab = () => {
  ...
  const save = (draft: Product) => {
    if (editor?.mode === "edit") {
      ...
      setProducts(list => [...list, { ...draft, id: nextId() }]);
    }
  };
    ...
    <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 14, padding: "18px 20px" }}>
      ...
      {count === 0 ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "48px 0", color: C.muted }}>
          ...
          <span style={{ fontSize: 13 }}>No data found.</span>
        </div>
      ) : (
