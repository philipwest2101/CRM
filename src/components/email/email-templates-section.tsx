...
import { EMAIL_TEMPLATES_STORE, JOURNEY_META, blocksToText, setEMAIL_TEMPLATES_STORE } from "../../lib/core";
import { C } from "../../theme";
...
export const EmailTemplatesSection = ({ navigateTo, role }) => {
  ...
  // ── CRUD (write-through to the unified store) ───────────────────────────────
  ...
  const addTemplate = () => {
    const lang = filterLang==="all" ? "de" : filterLang;
    const fresh = {
      id:`et-${Date.now()}`, lang,
      journey:filterJourney==="all"?"welcome":filterJourney,
      name:"New Template", subject:"",
      blocks:[{ type:"text", text: lang==="en"
        ? "Dear {{lead_name}},\n\n\n\nKind regards,\n{{advisor_name}}"
        : "Hallo {{lead_name}},\n\n\n\nFreundliche Grüße,\n{{advisor_name}}" }],
      attachments:[],
  ...
  return (
    <>
      ...
      <div style={{ marginBottom:16,padding:"10px 14px",borderRadius:9,background:C.blue+"06",
        ...
        All templates use the <strong style={{ color:C.blue }}>block editor</strong> — the same one as the Newsletter page.
        Journey templates drive workflow automation, manual sends and bulk campaigns;
        templates with the <strong>Newsletter</strong> journey also appear on the Newsletter page.
      </div>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:14,alignItems:"center",flexWrap:"wrap" }}>
        ...
        {/* Journey DDL */}
        {(()=>{ const active=filterJourney!=="all";
          return <div style={{ position:"relative" }}>
        ...
        {/* Clear all */}
        {(search||filterLang!=="all"||filterType!=="all"||filterActive!=="all"||filterJourney!=="all") && (
        ...
        <table style={{ width:"100%",minWidth:860,borderCollapse:"collapse",fontSize:12 }}>
          ...
          <tbody>
            ...
            {filtered.map((t,i) => {
              ...
              const preview  = blocksToText(t.blocks).replace(/\s+/g," ").replace(/\{\{[^}]+\}\}/g,"…");
                ...
                <tr key={t.id} style={{ borderBottom:`1px solid ${C.border}`,background:rowBg }}
                  ...
                  <td style={{ padding:"12px 14px",maxWidth:220 }}>
                    ...
                    <div style={{ fontSize:10,color:C.muted,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
                      🧱 {(t.blocks||[]).length} blocks{(t.attachments||[]).length>0 && ` · 📎 ${t.attachments.length}`}{preview && ` · ${preview.slice(0,45)}…`}
                    </div>
                  </td>
