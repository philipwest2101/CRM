    ...
    <div>
      ...
      <div style={{ marginBottom:28 }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14 }}>
          {[
            { id:"files",   icon:"📎", title:"Send Files",       desc:"Send PDFs, brochures or proposals directly to a lead's email.", action:"Compose & Send", color:C.blue   },
            { id:"collect", icon:"📥", title:"Collect Files",    desc:"Send the lead a secure upload link to collect their documents.", action:"Send Upload Link", color:C.green },
            { id:"invite",  icon:"🎟️", title:"Event Invitation", desc:"Invite a lead to a vion.world event, webinar or info evening.", action:"Send Invite", color:"#7C3AED" },
          ].map(card=>(
      ...
      {(role==="superadmin"||role==="vd") && (
        <div style={{ paddingTop:24,borderTop:`1px solid ${C.border}` }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
            {[["✅","Double Opt-In","Active",C.green,"Every welcome email includes a Double Opt-In link."],["🗑️","Data Retention","3 years",C.blue,"Lead data anonymised after retention period."],["📤","Right of Access","Available",C.green,"Export all data held on a lead as JSON/PDF."],["🚫","Right to Erasure","Available",C.red,"Delete a lead and all data with full audit trail."]].map(([ic,title,status,col,desc])=>(
      ...
      {mailingModal==="collect" && (<>
        ...
        <div style={{ position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:440,background:"#fff",borderRadius:16,zIndex:600,padding:"24px",boxShadow:"0 24px 64px rgba(0,0,0,0.2)",fontFamily:"inherit" }}>
          ...
          {mailSent ? <div style={{ padding:"20px",textAlign:"center",borderRadius:10,background:C.green+"0A",border:`1px solid ${C.green}30` }}><div style={{ fontSize:32,marginBottom:8 }}>✅</div><div style={{ fontSize:14,fontWeight:700,color:C.green }}>Upload link sent!</div></div> : (<>
            <div style={{ marginBottom:12 }}><label style={{ fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:"0.05em",display:"block",marginBottom:5 }}>Lead</label><input value={mailLead} onChange={e=>setMailLead(e.target.value)} placeholder="Search lead name…" style={{ width:"100%",padding:"9px 12px",borderRadius:8,border:`1.5px solid ${C.border}`,fontSize:13,fontFamily:"inherit",boxSizing:"border-box",outline:"none" }}/></div>
